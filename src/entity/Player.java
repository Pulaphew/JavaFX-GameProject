...
public class Player extends Entity implements UltimatePower, HealingPower {
	...
	public String heal() {
		...
		int healAmount = (int) (this.getMaxHealth() * 0.2);
		this.setCurrentHealth(this.getCurrentHealth() + healAmount);
		String[] dialogueHeal = { "You realize that life is not just about exams.",
				"Suddenly, the love from your father and mother comes to you.", "YOU SHOULDN'T LOST HERE,ARGHHHH!!!!" };
	...
	public String attack(Entity target, AttackZone sliderZone, EnemyPane enemyPane) {
		...
		if (chanceAttackHit < 1) {
			...
			// Since takeDamage(int damage) is void , cannot return String , then I put it
			// in this instead
			if (target instanceof Pta) {
				Pta enemyImmortal = (Pta) target;
				if (enemyImmortal.isImmortal()) {
					String dialogueCanNotDealDamage = enemyImmortal.getName()
							+ " is immortal right now! no damage taken.";
	...
	public String updateStatusEffects(int poisonDamage, PlayerPane playerPane) {
		...
		if (isPoisoned) {
			...
			dialogue += this.getName() + " takes " + poisonDamage + " poison damage! " + poisonTurnCount
					+ " turns left.\n";
